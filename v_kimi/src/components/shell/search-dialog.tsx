    ...
    <Dialog open={searchOpen} onOpenChange={setSearchOpen}>
      <DialogContent hideClose className="top-[20%] max-w-xl translate-y-0 gap-0 overflow-hidden p-0">
        ...
        <div className="flex items-center gap-2.5 border-b px-4">
          ...
          />
          <kbd className="rounded-md border bg-muted/60 px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground shadow-xs">
            ESC
          </kbd>
        </div>
          ...
          {results.length === 0 ? (
            ...
            results.map((c) => (
              <button
                ...
                className="group flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors hover:bg-primary/10"
              >
                <MessageSquareIcon className="size-4 shrink-0 text-muted-foreground transition-colors group-hover:text-primary" />
