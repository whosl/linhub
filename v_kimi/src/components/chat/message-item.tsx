...
export function MessageItem({
  ...
  if (message.role === "user") {
    return (
      ...
      >
        {message.quotedText && (
          <div className="max-w-[75%] rounded-t-2xl rounded-bl-2xl border-l-2 border-primary/50 bg-primary/[0.06] px-3 py-1.5 text-xs text-muted-foreground line-clamp-2">
            {message.quotedText}
          </div>
        )}
            ...
            return editing ? (
              <div key={i} className="w-full max-w-[85%]">
                <textarea
                  ...
                  className="glow-focus w-full resize-none rounded-2xl border bg-card px-4 py-3 text-[15px] shadow-[var(--shadow-card)] outline-none"
                />
              ...
              <div
                key={i}
                className="min-w-0 max-w-[85%] whitespace-pre-wrap rounded-[22px] rounded-br-md bg-brand px-4 py-2.5 text-[15px] leading-relaxed text-white shadow-[0_2px_10px_rgb(108_92_231/0.28)] [overflow-wrap:anywhere]"
              >
                {part.text}
              </div>
            );
        ...
        {/* 用户消息操作 */}
        {!editing && (
          <div className="flex items-center gap-0.5 opacity-100 transition-all duration-200 sm:translate-y-1 sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100 sm:group-focus-within:translate-y-0 sm:group-focus-within:opacity-100">
  ...
  return (
    ...
    >
      <div className="flex gap-3">
        {/* AI 头像：品牌渐变图标 */}
        <span className="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-lg bg-brand text-white shadow-[0_2px_8px_rgb(108_92_231/0.35)]">
            ...
            case "image": {
              ...
              return (
                ...
                <img
                  ...
                  alt={part.alt ?? "生成的图片"}
                  onClick={() => setLightbox({ src, alt: part.alt })}
                  className="my-3 max-h-96 cursor-zoom-in rounded-2xl border shadow-[var(--shadow-card)] transition-all duration-200 hover:shadow-[var(--shadow-lift)] hover:opacity-95"
                />
              );
            }
            default:
      ...
      {/* 助手消息操作栏 */}
      {showActionBar && (
        <div className="mt-1 flex items-center gap-0.5 opacity-100 transition-all duration-200 sm:translate-y-1 sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100 sm:group-focus-within:translate-y-0 sm:group-focus-within:opacity-100">
          {branch && branch.total > 1 && <BranchSwitcher branch={branch} />}
          {hasCopyableText && (
            <Tooltip label={copied ? "已复制" : "复制"}>
