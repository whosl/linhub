  ...
  return (
    <PageContainer wide>
      <PageHeader
        ...
        action={
          <Button className="btn-brand" onClick={() => openEditor()}>
            <PlusIcon /> 创建技能
          </Button>
        }
      />
        ...
        <TabsContent value="mine">
          {mySkills.length === 0 ? (
            <EmptyState
              ...
              action={<Button className="btn-brand" onClick={() => openEditor()}>创建第一个技能</Button>}
            />
          ) : (
            <SkillGrid
              skills={mySkills}
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {skills.map((s, i) => (
        ...
        >
          <Card className="card-elevated group flex h-full flex-col p-5">
            <div className="mb-3 flex items-start justify-between">
              <span className="flex size-11 items-center justify-center rounded-xl bg-gradient-to-br from-primary/15 to-accent text-2xl shadow-sm transition-transform duration-300 group-hover:scale-105">
                {s.emoji}
              </span>
              ...
              <div className="flex gap-1 opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100 sm:focus-within:opacity-100">
                ...
                {!isInternalRuntime(s) && (
                  <button
                    type="button"
                    onClick={() => onChat(s)}
                    className="btn-brand flex items-center gap-1 rounded-md px-2 py-1.5 font-medium"
                  >
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <div className="flex items-center gap-3 pr-8">
            <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-primary/15 to-accent text-2xl shadow-sm" aria-hidden>{skill.emoji}</span>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-xl">
        ...
        <DialogFooter>
          ...
          <Button className="btn-brand" onClick={save} disabled={!form.name.trim() || !form.systemPrompt.trim()}>
            保存
          </Button>
        </DialogFooter>
      </DialogContent>
