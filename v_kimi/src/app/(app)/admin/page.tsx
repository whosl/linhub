  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end">
        <Button size="sm" className="btn-brand" onClick={() => openEditor()}>
          <PlusIcon /> 添加供应商
        </Button>
      </div>
      {providers.map((p) => (
        <Card key={p.id} className="flex items-center gap-3 p-4 shadow-[var(--shadow-card)] transition-shadow hover:shadow-[var(--shadow-lift)]">
          <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-primary/15 to-accent text-primary">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2 md:hidden">
        {models.map((m) => (
          <Card key={m.id} className="space-y-3 p-3 shadow-[var(--shadow-card)]">
      ...
      <Card className="hidden overflow-x-auto shadow-[var(--shadow-card)] md:block">
...
function PlansAdminTab() {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
      {plans.map((p) => (
        <Card key={p.id} className="p-5 shadow-[var(--shadow-card)] transition-shadow hover:shadow-[var(--shadow-lift)]">
...
function UsersTab() {
  ...
  return (
    ...
    {selectedUserId ? (
      ...
      <Card className="divide-y overflow-hidden shadow-[var(--shadow-card)]">
        {users.map((u) => (
          <div
            key={u.id}
            className="flex flex-col gap-3 px-4 py-3 transition-colors hover:bg-accent/30 sm:flex-row sm:items-center sm:px-5"
          >
            <button
              type="button"
...
function UserDetailView({
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="space-y-3 border-destructive/30 p-5 shadow-[var(--shadow-card)]">
        <div>
...
function UserUsageLog({ records }: { records: UsageRecord[] }) {
  if (records.length === 0) {
    return <Card className="p-5 text-sm text-muted-foreground shadow-[var(--shadow-card)]">暂无用量记录</Card>;
  }
  return (
    <Card className="divide-y overflow-hidden shadow-[var(--shadow-card)]">
      {records.map((record) => (
...
function UserLedgerLog({ entries }: { entries: LedgerEntry[] }) {
  if (entries.length === 0) {
    return <Card className="p-5 text-sm text-muted-foreground shadow-[var(--shadow-card)]">暂无余额流水</Card>;
  }
  return (
    <Card className="divide-y overflow-hidden shadow-[var(--shadow-card)]">
      {entries.map((entry) => (
...
function SkillReviewTab() {
  ...
  return (
    <div className="space-y-3">
      {pending.map((s) => (
        <Card key={s.id} className="flex items-center gap-3 p-4 shadow-[var(--shadow-card)] transition-shadow hover:shadow-[var(--shadow-lift)]">
          <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-primary/15 to-accent text-xl shadow-sm">
            {s.emoji}
          </span>
          <div className="min-w-0 flex-1">
          ...
          <Button size="sm" className="btn-brand" onClick={() => review(s.id, true)}>
            <CheckIcon /> 通过
          </Button>
        </Card>
      ))}
...
function EngineCard({
  ...
  return (
    <Card
      className={cn(
        "space-y-4 p-4 shadow-[var(--shadow-card)] transition-colors",
        ...
        {globalMcp.length === 0 ? (
          ...
          <div className="space-y-2">
            {globalMcp.map((s) => (
              <div
                key={s.id}
                className="group rounded-xl border bg-background px-3.5 py-2.5 transition-colors hover:border-primary/30"
              >
