  ...
  return (
    <PageContainer wide>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card className="bg-brand relative overflow-hidden border-transparent p-5 text-white shadow-[var(--shadow-lift)]">
          {/* 装饰光斑 */}
      ...
      {plans.map((p) => {
        ...
        return (
          <Card
            key={p.id}
            className={cn(
              "relative flex flex-col p-6 shadow-[var(--shadow-card)] transition-shadow hover:shadow-[var(--shadow-lift)]",
              highlight && "border-primary/50 shadow-[var(--glow-primary)]"
            )}
          >
            {highlight && (
              <Badge className="bg-brand absolute -top-2.5 left-1/2 -translate-x-1/2 text-white shadow-md">
                最受欢迎
              </Badge>
            )}
            ...
            <Button
              className={cn("mt-5", !isCurrent && highlight && "btn-brand")}
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      <Card className="p-6 shadow-[var(--shadow-card)]">
        ...
        <Button className="btn-brand mt-4 w-full" onClick={recharge} disabled={recharging}>
      ...
      <Card className="p-6 shadow-[var(--shadow-card)]">
        ...
        <div className="flex gap-2">
          ...
          />
          <Button className="btn-brand" onClick={redeem} disabled={!code.trim() || redeeming}>
            {redeeming ? "兑换中…" : "兑换"}
          </Button>
        </div>
...
function LedgerTab() {
  ...
  return (
    <Card className="divide-y overflow-hidden shadow-[var(--shadow-card)]">
      {entries.map((e) => (
        <div key={e.id} className="flex items-center justify-between px-5 py-3 transition-colors hover:bg-accent/30">
          <div>
