    ...
    <PageContainer wide>
      ...
      {kbs.length === 0 ? (
        <EmptyState
          ...
          action={<Button className="btn-brand" onClick={() => setCreateOpen(true)}>创建第一个知识库</Button>}
        />
      ) : (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="-mx-4 overflow-x-auto px-4 pb-1 [scrollbar-width:none] lg:mx-0 lg:overflow-visible lg:px-0 lg:pb-0 [&::-webkit-scrollbar]:hidden">
            ...
            {kbs.map((kb, i) => (
              <motion.div
                ...
                className={cn(
                  "group w-64 shrink-0 cursor-pointer rounded-2xl border bg-card p-4 text-left shadow-[var(--shadow-card)] transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-[var(--shadow-lift)] lg:w-full",
                  selected?.id === kb.id && "border-primary/60 ring-1 ring-primary/30"
                )}
              >
                <div className="flex items-start justify-between">
                  <span className="mb-2 flex size-9 items-center justify-center rounded-lg bg-gradient-to-br from-primary/15 to-accent text-primary">
          ...
          <div className="lg:col-span-2">
            {!selected ? (
              ...
              <Card
                className={cn(
                  "p-5 shadow-[var(--shadow-card)] transition-all",
                  dragActive && "border-primary/60 bg-primary/5 shadow-[var(--glow-primary)]"
                )}
                {...dropHandlers}
              >
                <div className="mb-4 flex items-center justify-between">
                  ...
                  />
                  <Button size="sm" className="btn-brand" onClick={openFilePicker} disabled={uploading}>
                    {uploading ? (
                ...
                {documents.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {documents.map((doc, docIndex) => {
                      ...
                      <div
                        key={doc.id}
                        className="group flex items-center gap-3 rounded-xl border px-3.5 py-3 transition-colors hover:border-primary/30 hover:bg-accent/30"
                      >
                        {isImage ? (
                        ...
                        {doc.status === "processing" ? (
                          ...
                          <Badge variant="warning" title={doc.errorMessage}>
                            解析失败
                          </Badge>
                        ) : (
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button className="btn-brand" onClick={create} disabled={!name.trim()}>
              创建
            </Button>
          </DialogFooter>
        </DialogContent>
