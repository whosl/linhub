...
metadata:
  ...
  version: "1.0.0"
  category: code
allowed-tools: run_code start_code_lab
---

# 代码实验室

通过 `run_code` 或后台 `start_code_lab` 在临时 gVisor 沙盒中执行代码。不要展示或声称存在未实际得到的运行结果。
...
4. 普通任务调用 `run_code`；数据和压缩包处理把超时设为 120 秒。预计超过 120 秒、数据集较大或用户需要刷新恢复/停止能力时调用 `start_code_lab`，最长 300 秒。
5. Python 处理 ZIP/TAR/GZIP 时优先使用 `linhub_safe_archive.safe_extract_zip`、`safe_extract_tar` 或 `safe_extract_gzip`；它们会检查成员路径、文件数和解压后总大小，并只允许解压到 `/tmp`。
6. 检查退出码、stderr、超时和输出限制。失败时根据真实错误修正代码后最多重试两次。
7. 用简短结论说明执行了什么、结果是否通过，并把工具返回的输出附件交付给用户。

## 约束

- 不要通过网络下载安装依赖；Python 镜像已预装 pandas、NumPy、PyArrow、OpenPyXL、Polars、DuckDB、SciPy、scikit-learn、Matplotlib、Seaborn 和 XlsxWriter。
